...
public class TableInfoHandler {
            ...
            for (Integer pageNumber : bplusTree.getLeafNodes()) {
                ...
                for (DataRecordForTable tableRecord : currentPage.getPageRecords()) {
                    ...
                    if (tableRecord.getColumnsList().get(0).fieldValue.equals(tableName)) {
                        this.rootPageNumber = Integer.parseInt(tableRecord.getColumnsList().get(3).fieldValue);
                        recordsCount = Integer.parseInt(tableRecord.getColumnsList().get(1).fieldValue);
                        tableExists = true;
                        break;
                    }
                }
                    ...
                    if (tableRecord.getColumnsList().get(0).fieldValue.equals(tableName)) {
                        {
                            ...
                            columnNamesList.add(tableRecord.getColumnsList().get(1).fieldValue);
                            ColumnValueConstrain colInfo = new ColumnValueConstrain(
                                    tableName
                                    , DBSupportedDataType.get(tableRecord.getColumnsList().get(2).fieldValue)
                                    , tableRecord.getColumnsList().get(1).fieldValue
                                    , tableRecord.getColumnsList().get(6).fieldValue.equals("YES")
                                    , tableRecord.getColumnsList().get(4).fieldValue.equals("YES")
                                    , Short.parseShort(tableRecord.getColumnsList().get(3).fieldValue)
                            );

                            if (tableRecord.getColumnsList().get(5).fieldValue.equals("PRI"))
    ...
    public boolean columnExists(List<String> columnsList) {
        ...
        for (ColumnValueConstrain column_name_attr : columnNameAttrList) {
            leftColumnsList.remove(column_name_attr.colname);
        }
    ...
    public boolean validateInsert(List<CellRecords> rowAttributesList) throws IOException {
        ...
        for (int i = 0; i < columnNameAttrList.size(); i++) {
            ...
            condition.columnName = columnNameAttrList.get(i).colname;
            condition.columnOrdinal = i;
            condition.setOperator("=");

            if (columnNameAttrList.get(i).isSame) {
                condition.setConditionValue(rowAttributesList.get(i).fieldValue);
                if (file.checkIfRecordExists(this, condition)) {
                    System.out.println("! Insert failed: Column " + columnNameAttrList.get(i).colname + " should be unique.");
                    operatingTableFile.close();
                    return false;
                }
            }
