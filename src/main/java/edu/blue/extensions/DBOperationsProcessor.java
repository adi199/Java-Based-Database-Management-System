...
public class DBOperationsProcessor {
    ...
    public boolean checkIfRecordExists(TableInfoHandler tableMetaData, WhereConditionProcessor condition) throws IOException {
        ...
        for (Integer pageNo : bPlusTree.getAllLeaveNodes(condition)) {
    ...
    public int updateRecords(TableInfoHandler tableMetaData, WhereConditionProcessor condition,
        ...
        int optionalIndex = -1;
        for (Integer pageNo : bPlusOneTree.getAllLeaveNodes(condition)) {
            ...
            for (DataRecordForTable record : page.getPageRecords()) {
                ...
                List<CellRecords> attributeList = record.getAttributeList();
                for (int i : newValueMap.keySet()) {
                    CellRecords oldValue = record.getAttributeList().get(i);
                    int rowId = record.rowId;
                    if ((record.getAttributeList().get(i).dataType == DBSupportedDataType.TEXT
                            && record.getAttributeList().get(i).fieldValue.length() == newValueMap.get(i).fieldValue.length())
                            || (record.getAttributeList().get(i).dataType != DBSupportedDataType.NULL && record.getAttributeList().get(i).dataType != DBSupportedDataType.TEXT)
                    ) {
                        ...
                        attr = newValueMap.get(i);
                        attributeList.add(i, attr);
                        int pageNumberForInsertion = BPlusTree.getPageNumberForInsertion(file, tableMetaData.rootPageNumber);
                    ...
                    if (tableMetaData.columnNameAttrList.get(i).hasIndex && condition != null
                    ) {
                        RandomAccessFile indexFile = new RandomAccessFile(Utils.getIndexFilePath(tableMetaData.columnNameAttrList.get(i).tableName, tableMetaData.columnNameAttrList.get(i).columnName), "rw");
                        BTree bTree = new BTree(indexFile);
                        bTree.delete(oldValue, record.rowId);
                        bTree.insert(newValueMap.get(i), rowId);
                        indexFile.close();
                    }
                    if (optionalIndex != -1 && tableMetaData.columnNameAttrList.get(optionalIndex).hasIndex) {
                        RandomAccessFile indexFile = new RandomAccessFile(Utils.getIndexFilePath(tableMetaData.columnNameAttrList.get(optionalIndex).tableName, tableMetaData.columnNameAttrList.get(optionalIndex).columnName), "rw");
                        BTree bTree = new BTree(indexFile);
                        bTree.delete(record.getAttributeList().get(optionalIndex), record.rowId);
                        bTree.insert(record.getAttributeList().get(optionalIndex), rowId);
                        indexFile.close();
                    }
    ...
    public void selectRecordsFromTable(TableInfoHandler tableMetaData, List<String> columNames, WhereConditionProcessor condition) throws IOException {
        ...
        for (int i : indexPositionList) {
            String columnName = tableMetaData.columnNameAttrList.get(i).columnName;
        ...
        String currentValue;
        int count = 0;
        for (Integer workingPageNumber : bPlusTree.getAllLeaveNodes(condition)) {
            Page page = new Page(file, workingPageNumber);
            for (DataRecordForTable tableRecord : page.getPageRecords()) {
                if (condition != null) {
                    if (!condition.checkCondition(tableRecord.getAttributeList().get(condition.columnOrdinal).fieldValue))
                        continue;
                }
                int columnCount = 0;
                if (showRowId) {
                    currentValue = Integer.valueOf(tableRecord.rowId).toString();
                ...
                for (int w : indexPositionList) {
                    currentValue = tableRecord.getAttributeList().get(w).fieldValue;
                    System.out.print(currentValue);
                    System.out.print(Utils.printSeparator(" ", printingPositionList.get(++columnCount) - currentValue.length()));
                }
                System.out.println();
