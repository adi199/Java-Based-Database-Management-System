...
public class Page {
    ...
    public void updateRecord(DataRecordForTable record, int ordinalPosition, Byte[] newValue) throws IOException {
        binaryFile.seek(start + record.OffsetofRecord + 7);
        int voffset = 0;
        ...
        binaryFile.seek(start + record.OffsetofRecord + 7 + record.columnDataTypes.length + voffset);
    ...
    public void addNewColumn(ColumnValueConstrain columnInfo) {
        try {
            addTableRow(DBOperationsProcessor.columnsTable, Arrays.asList(new CellRecords[]{
                    new CellRecords(DBSupportedDataType.TEXT, columnInfo.tableName),
                    new CellRecords(DBSupportedDataType.TEXT, columnInfo.columnName),
                    new CellRecords(DBSupportedDataType.TEXT, columnInfo.dataType.toString()),
                    new CellRecords(DBSupportedDataType.SMALLINT, columnInfo.ordinalPosition.toString()),
                    new CellRecords(DBSupportedDataType.TEXT, columnInfo.isNullable ? "YES" : "NO"),
                    columnInfo.isPrimaryKey ?
                            new CellRecords(DBSupportedDataType.TEXT, "PRI") : new CellRecords(DBSupportedDataType.NULL, "NULL"),
                    new CellRecords(DBSupportedDataType.TEXT, columnInfo.isUnique ? "YES" : "NO")
            }));
        } catch (Exception e) {
        ...
        if (pageType == PageNodeType.LEAF_INDEX) {
            ...
            if (noOfCells < 3 && !idxPageCleaned) {
                ...
                for (String s : indexValuesTemp) {
                    addIndex(pointerTemp.get(s).getIndexNodeReference(), pointerTemp.get(s).leftPageNumber);
                }
    ...
    private IndexNode splitIndexRecordsBetweenPages(Page newleftPage) throws IOException {
        ...
        try {
            ...
            IndexNode toInsertParentIndexNode = pointer.get(indexValuesTemp[mid]).getIndexNodeReference();
            toInsertParentIndexNode.leftPageNumber = pointer.get(indexValuesTemp[mid]).leftPageNumber;

            HashMap<String, IndexRecord> pointerTemp = (HashMap<String, IndexRecord>) pointer.clone();

            for (int i = 0; i < mid; i++) {
                newleftPage.addIndex(pointerTemp.get(indexValuesTemp[i]).getIndexNodeReference(), pointerTemp.get(indexValuesTemp[i]).leftPageNumber);
            }
            ...
            for (int i = mid + 1; i < indexValuesTemp.length; i++) {
                addIndex(pointerTemp.get(indexValuesTemp[i]).getIndexNodeReference(), pointerTemp.get(indexValuesTemp[i]).leftPageNumber);
            }
    ...
    private void fillIndexRecords() {
        try {
            ...
            for (short i = 0; i < noOfCells; i++) {
                ...
                if (inType == DBSupportedDataType.TEXT || inType == null || record.getIndexNodeReference().indexValue.fieldValue.toUpperCase().equals("NULL"))
                    sIndexValues.add(record.getIndexNodeReference().indexValue.fieldValue);
                else
                    lIndexValues.add(Long.parseLong(record.getIndexNodeReference().indexValue.fieldValue));

                pointer.put(record.getIndexNodeReference().indexValue.fieldValue, record);
